...
void DamSim<SensorProvider>::write_data(Dam &v, int cur_timestep)
{
  ...
  int water_level = v.get_water_level();
  ofstream ost{"out.csv", ios_base::app};  //Antar vi skal bruke denne filen. Bruker app for å legge til data i filen.
  //Sjekker ikke om man ikke får åpnet filen siden det ikke stod i oppgaven.
  ost << name << "," << cur_timestep << "," << water_level << "\n";
  // END: F1
}
