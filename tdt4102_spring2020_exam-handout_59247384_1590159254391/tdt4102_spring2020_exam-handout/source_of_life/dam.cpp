...
{
  // BEGIN: D2
  if (water_level< water_level_min){return WaterAlarm::WaterLevelLow;}
  else if (water_level > water_level_max){return WaterAlarm::WaterLevelHigh;}
...
{
  // BEGIN: D3  //Gjør kun endringer hvis vannstanden er utenfor de kritiske veridene. Antar vi skal bruke inflow_value og outflow_value
                //til å endre water_level.
  if (water_level_delta() < inflow_capacity || water_level_delta() < outflow_capacity){
    water_level = ideal_level;
  }else if (get_alarm()==WaterAlarm::WaterLevelLow){
    water_level += inflow_value;
  }else if(get_alarm()==WaterAlarm::WaterLevelHigh){
    water_level -= outflow_value;
  }  //Siste if-mulighet er NoAlarm, da gjør vi ingenting.
  // END: D3
}
