...
Color::Color_type GraphColor::next_color()
{
  // BEGIN: P1
  cur_color += 1;
  if (cur_color >= colors.size()){
    throw "Error: No more colors";
  }
  return colors.at(cur_color-1); 
  // END: P1
}
...
{
  // BEGIN: P2
  for (unsigned int i=0; i<colors.size()-2; ++i){  //Bruker algoritmen beskrevet ovenfor
...
/* Assignment P3
  ...
  Implement the function get_color which takes the name of a dam as an
...
{
  // BEGIN P3
  if (dam_colors.find(dam) == dam_colors.end()){
    dam_colors[dam] = colors.next_color();
  }
  return dam_colors[dam];
  // END P3
}
...
Point Plotter::get_last_point(const string dam, Point current_point)
{
  // BEGIN: P4
  Point p;
  if (last_points.find(dam) == last_points.end()){
    p = {0,0};
  }else{
    p = last_points[dam];
  }
  last_points[dam] = current_point;
  return p;
  // END: P4
}
...
{
  // BEGIN: P5
  if (alarm == Dam::WaterAlarm::NoAlarm){return;}
  int aLineHeight = y_len/15;  //Setter høyden til alarmlinjen til å være en 15-del av y-aksen.
  shapes.push_back(new Line{Point{calc_x(timestep),origin.y-aLineHeight/2},Point{calc_x(timestep),origin.y+aLineHeight/2}});
  shapes[shapes.size()-1]->set_color(get_color(dam));
...
{
  // BEGIN: P6
  shapes.push_back(new Line{Point{origin.x,origin.y-static_cast<int>(limit)},Point{origin.x + x_len,origin.y-static_cast<int>(limit)}});
  shapes[shapes.size()-1]->set_color(get_color(dam));
  shapes[shapes.size()-1]->set_style(Line_style::dash);
  attach(*(shapes[shapes.size()-1]));
  // END: P6
}
